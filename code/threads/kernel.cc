...
void Kernel::PrintInt(int number) //implement PrintInt in Kernel.h
{
    synchConsoleOut->PrintInt(number); //call console through SynchConsole to synchronous it
}

OpenFileId Kernel::Open(char *name) //implement Open in Kernel.h
{
    return (OpenFileId)fileSystem->Open(name); //call Open in fileSystem, the return type is a pointer of OpenFile, so transform the pointer into OpenFileId
}

int Kernel::Write(char *buffer, int size, OpenFileId id) //implement Write in Kernel.h
{
    return fileSystem->Write(buffer, size, id); //call Write in fileSystem
}

int Kernel::Read(char *buffer, int size, OpenFileId id) //implement Read in Kernel.h
{
    return fileSystem->Read(buffer, size, id); //call Read in fileSystem
}

int Kernel::Close(OpenFileId id) //implement Close in Kernel.h
{
    return fileSystem->Close(id); //call Close in fileSystem
}
